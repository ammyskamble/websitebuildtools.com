  ...
  return (
    <section className="py-16 max-w-5xl mx-auto px-4 sm:px-6" id="faq-section">
      ...
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-brand-500/10 border border-brand-500/20 text-brand-600 dark:text-brand-400 text-xs font-medium mb-3">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 mt-2 max-w-2xl mx-auto leading-relaxed">{description}</p>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-8 p-3 rounded-2xl glass-card border border-dark-border">
        ...
        <div className="flex flex-wrap items-center gap-1.5 text-xs">
          <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider px-2">Topic:</span>
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-medium transition-all ${
                selectedCategory === cat.id
                  ...
                  : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-dark-hover/50'
              }`}
            >
              {cat.label}
            </button>
        ...
        <div className="flex items-center gap-1.5 text-xs">
          <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider px-1">Language:</span>
          {[
            ...
            >
              ...
              <summary className="list-none w-full cursor-pointer px-6 py-4 flex items-center justify-between gap-4 select-none hover:bg-slate-100/50 dark:hover:bg-dark-hover/30 transition-colors">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  {seoItem.language && (
                    <span className="shrink-0 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {(hasPhotoshop || hasIllustrator) && (
                    <span className="hidden sm:inline-flex items-center gap-1 text-[11px] text-brand-600 dark:text-brand-400 bg-brand-500/10 px-2 py-0.5 rounded-full border border-brand-500/20">
                      <Sparkles className="w-3 h-3" />
                      <span>Ps / Ai Guide</span>
                    </span>
                  )}
                  {hasFormula && (
                    <span className="hidden md:inline-flex items-center gap-1 text-[11px] text-amber-600 dark:text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full border border-amber-500/20">
                      <Calculator className="w-3 h-3" />
                      <span>Math Formula</span>
                    </span>
                  )}
                  <ChevronDown className="w-4 h-4 text-slate-400 group-open:rotate-180 group-open:text-brand-600 dark:group-open:text-brand-400 transition-transform duration-200" />
                </div>
              </summary>

              {/* Collapsible Content */}
              <div className="px-6 pb-6 pt-2 border-t border-dark-border/40 text-xs sm:text-sm text-slate-700 dark:text-slate-300 space-y-5 animate-in fade-in">
                {/* Core Answer */}
                ...
                {hasFormula && seoItem.mathFormula && (
                  <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/20 space-y-2">
                    ...
                    <div className="p-3 rounded-lg bg-amber-100/60 dark:bg-black/40 border border-amber-300 dark:border-amber-500/20 font-mono text-xs sm:text-sm text-amber-900 dark:text-amber-200 overflow-x-auto whitespace-pre-wrap">
                ...
                {(hasPhotoshop || hasIllustrator) && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
                    ...
                    {hasIllustrator && (
                      <div className="p-4 rounded-xl bg-amber-50 dark:bg-[#1a0a03]/50 border border-amber-300 dark:border-amber-600/30 space-y-2.5">
                        <div className="flex items-center justify-between">
                          <span className="flex items-center gap-1.5 font-bold text-amber-700 dark:text-amber-400 text-xs">
                            <span className="w-5 h-5 rounded bg-amber-500/20 text-amber-800 dark:text-amber-300 flex items-center justify-center font-bold text-[10px] border border-amber-500/30">
                              Ai
                            </span>
                            <span>Adobe Illustrator Workflow</span>
                          </span>
                          <span className="text-[10px] text-amber-700 dark:text-amber-400/80 bg-amber-500/10 px-2 py-0.5 rounded">Vector Native</span>
                        </div>
                        <ol className="list-decimal list-inside space-y-1.5 text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
                          {seoItem.illustratorSteps!.map((step, sIdx) => (
                            <li key={sIdx} className="text-slate-700 dark:text-slate-300">
                    ...
                    {hasPhotoshop && (
                      <div className="p-4 rounded-xl bg-blue-50 dark:bg-[#031528]/50 border border-blue-300 dark:border-blue-600/30 space-y-2.5">
                        <div className="flex items-center justify-between">
                          <span className="flex items-center gap-1.5 font-bold text-blue-700 dark:text-blue-400 text-xs">
                            <span className="w-5 h-5 rounded bg-blue-500/20 text-blue-800 dark:text-blue-300 flex items-center justify-center font-bold text-[10px] border border-blue-500/30">
                              Ps
                            </span>
